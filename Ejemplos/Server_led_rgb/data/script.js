...
function createSwatch(target, color) {
  ...
  swatch.addEventListener("click", function () {
    ...
    sendData(red.value, green.value, blue.value);
  });
...
function sendData(red, green, blue) {
  ...
  const url = "http://192.168.1.75:3001/color";
  fetch(url, {
    method: "POST",
...
var startGetSpectrumColor = function (e) {
  ...
  window.addEventListener("touchstart", endGetSpectrumColor);
};
...
function startGetHueColor(e) {
  ...
  window.addEventListener("touchmove", getHueColor);
  window.addEventListener("touchstart", endGetHueColor);
}
...
modeToggle.addEventListener("click", function () {
  if (
    rgbFields.classList.contains("active") ? rgbFields.classList.remove("active") : rgbFields.classList.add("active")
  );
